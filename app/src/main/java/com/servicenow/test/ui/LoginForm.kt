...
class LoginForm : BaseObservable() {
    ...
    val isValid: Boolean
        get() {
            if(!firstNameField.validate && !lastNameField.validate)
                return true
            var valid = isValidInfutField(firstNameField)
            valid = isValidInfutField(lastNameField) && valid
    ...
    fun isValidInfutField(loginField: LoginField?) : Boolean {
        ...
        if(empty) loginField?.error = "This field cannot be left blank"
        else loginField?.error = null
        return loginField?.error === null
    }
    ...
    val firstNameError: String?
        get() = firstNameField.error

    @get:Bindable
    val lastNameError: String?
        get() = lastNameField.error
